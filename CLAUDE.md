...
| `ai_provider.py` | Optional LLM layer for the coach. Detects provider from the API-key prefix (`sk-ant-…` → Anthropic, `xai-…` → xAI Grok, `sk-…` → OpenAI), validates keys via GET `/v1/models`, builds a compact portfolio-context JSON via `build_portfolio_context(...)`, and POSTs via `requests`. `LLM_PROMPTS` is the list of 6 deeper preset questions shown once a key is saved. |
...
- **AI Coach panel** — "✨ Ask" button in the Holdings header toggles `coach-open`; a close button inside the panel also writes `coach-open=False`. The `render_coach` callback owns the full `coach-panel` children as one unified card (header + thread tabs + chat + input row) based on five stores: `coach-open`, `coach-mode`, `coach-active-id`, `coach-api-key` (browser localStorage), `coach-key-status`, plus `coach-threads` / `coach-active-thread-id` (both localStorage) and the three data stores as State. Threads (named conversations) are persisted in `coach-threads`; `_derive_chat_history` derives `coach-chat-history` from the active thread so `render_chat` can update only the chat area without rebuilding the whole panel. API key is validated via `ai_provider.validate_key()` before saving. LLM call runs synchronously inside `run_llm` (triggered by `coach-pending-q`). When the panel isn't open, child components don't exist — all coach callbacks rely on `suppress_callback_exceptions=True`.
